...
onMounted(() => {
  try {
    if (userData.value) {
      const parsed = typeof userData.value === 'string'
...
const {
  ...
  `user-${route.params.id}-orders`,
  async () => {
    if (!loadingAuth.value && !isAuthenticated.value) {
  ...
  },
  {
    server: false,
    immediate: true,
    watch: [ isAuthenticated ] // 使用 ref 來監聽
  }
);
...
<template>
  <div>
    ...
    <div v-if="orderList" class="row gap-6 gap-md-0">
      <div class="col-12 col-md-7">
        <div class="rounded-3xl d-flex flex-column gap-6 gap-md-10 p-4 p-md-10 bg-neutral-0"
          ...
          <img class="img-fluid rounded-3" :src="orderList[ 0 ].roomId.imageUrl" alt="room-a">

          <section class="d-flex flex-column gap-6">
            <h3 class="d-flex align-items-center mb-0 text-neutral-80 fs-8 fs-md-6 fw-bold">
              <p class="mb-0">
                {{ orderList[ 0 ].roomId.name }}，{{ countNights(orderList[ 0 ].checkInDate,
                  orderList[ 0 ].checkOutDate) }} 晚
              </p>
              <span class="d-inline-block mx-4 bg-neutral-80" style="width: 1px;height: 18px;" />
              <p class="mb-0">
                住宿人數：{{ orderList[ 0 ].peopleNum }} 位
              </p>
            </h3>

            <div class="text-neutral-80 fs-8 fs-md-7 fw-bold">
              <p class="title-deco mb-2">
                入住：{{ new Date(orderList[ 0 ].checkInDate).toLocaleDateString('zh-TW', {
                  month:
                    'long', day: 'numeric', weekday: 'long'
                }) }}，15:00 可入住
              </p>
              <p class="title-deco mb-0">
                退房：{{ new Date(orderList[ 0 ].checkOutDate).toLocaleDateString('zh-TW', {
                  month:
                    'long', day: 'numeric', weekday: 'long'
                }) }}，12:00 前退房
              </p>
            ...
            <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
              NT$ {{ formatCurrency(orderList[ 0 ].roomId.price * countNights(orderList[ 0
              ].checkInDate,
                orderList[ 0 ].checkOutDate) - 1000) }}
            </p>
          </section>
          ...
          <div class="d-flex gap-4">
            ...
            <NuxtLink :to="`/rooms/${orderList[ 0 ].roomId._id}`"
      ...
      <div class="col-12 col-md-5">
        <div class="rounded-3xl d-flex flex-column gap-6 gap-md-10 p-4 p-md-10 bg-neutral-0">
          ...
          <div v-for="(order, index) in orderList" :key="order._id">
            <div class="d-flex flex-column flex-lg-row gap-6">
              ...
              <section class="d-flex flex-column gap-4">
                ...
                <div class="text-neutral-80 fw-medium">
                  <p class="mb-2">
                    住宿天數： {{ countNights(order.checkInDate, order.checkOutDate) }} 晚
                  </p>
                  <p class="mb-0">
                ...
                <div class="text-neutral-80 fs-8 fs-md-7 fw-medium">
                  <p class="title-deco mb-2">
                    入住：{{ new Date(order.checkInDate).toLocaleDateString('zh-TW', {
                      month:
                        'long', day: 'numeric', weekday: 'long'
                    }) }}，15:00 可入住
                  </p>
                  <p class="title-deco mb-0">
                    退房：{{ new Date(order.checkOutDate).toLocaleDateString('zh-TW', {
                      month:
                        'long', day: 'numeric', weekday: 'long'
                    }) }}，12:00 前退房
                  </p>
                ...
                <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                  NT$ {{ formatCurrency(order.roomId.price * countNights(order.checkInDate,
                    order.checkOutDate) -
                    1000) }}
                </p>
              </section>
            ...
            <hr class="my-0 opacity-100 text-neutral-40"
                :class="{ 'd-none': orderList.length < 2 || index === orderList.length - 1 }">
          </div>

          <button class="btn btn-outline-primary-100 py-4 fw-bold"
                  style="--bs-btn-hover-color: #fff" type="button">
            查看更多
          </button>
        </div>
