...
const fetchRoomDetailData = async (id) => {
  try {
    loading.value = true;
console.log('id', id);
  const roomDetailResponse = await api.getRoomDetail(id);
  if (roomDetailResponse.error.value) {
      throw new Error(roomDetailResponse.error.value.message);
    }
  roomDetail.value = roomDetailResponse.data.value?.result;
  MAX_BOOKING_PEOPLE.value = roomDetailResponse.data.value?.result.maxPeople;
...
onMounted(async () => {
await fetchRoomDetailData(roomId);
});
        ...
        <div class="d-flex gap-2 rounded-3xl overflow-hidden">
          ...
          >
            <div class="d-flex gap-md-2">
              <img
                v-for="image in roomDetail.imageUrlList.slice(0, 2)" :key="image"
                class="w-50"
                :src="image"
                alt="room image"
              >
            ...
            <div class="d-flex gap-md-2">
              <img
                v-for="image in roomDetail.imageUrlList.slice(2, 4)" :key="image"
                class="w-50"
                :src="image"
                alt="room image"
              >
    ...
    <section class="py-10 py-md-30 bg-primary-10">
          ...
          <div class="col-12 col-md-7 d-flex flex-column gap-6 gap-md-20">
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap gap-6 gap-md-10 p-6 bg-neutral-0 fs-8 fs-md-7 rounded-3 list-unstyled">
                <li v-for="layout in roomDetail.layoutInfo" :key="layout.title" class="d-flex gap-2">
                  <Icon
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 bg-neutral-0 fs-8 fs-md-7 rounded-3 list-unstyled">
                <li v-for="facility in roomDetail.facilityInfo" :key="facility.title" class="flex-item d-flex gap-2">
                  <Icon
                    class="fs-5 text-primary-100"
                    icon="material-symbols:check"
                  />
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 bg-neutral-0 fs-8 fs-md-7 rounded-3 list-unstyled">
                <li v-for="amenity in roomDetail.amenityInfo" :key="amenity.title" class="flex-item d-flex gap-2">
                  <Icon
                    class="fs-5 text-primary-100"
                    icon="material-symbols:check"
                  />
      ...
      <div class="d-flex d-md-none justify-content-between align-items-center position-fixed bottom-0 w-100 p-3 bg-neutral-0">
        <template v-if="bookingDate.date.end === null">
          <small class="text-neutral-80 fw-medium">ＮＴ$ {{ formatCurrency(roomDetail.price) }} / 晚</small>
          <button
        ...
        <template v-else>
          <div class="d-flex flex-column gap-1">
            <small class="text-neutral-80 fw-medium">ＮＴ$ {{ formatCurrency(roomDetail.price) }} / {{ daysCount }} 晚 / {{ bookingPeople }} 人</small>
